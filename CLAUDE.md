...
- フェーズの進め方：
  ...
  | ステップ | 担当 | 内容 |
  |---------|------|------|
  | 計画（必須ゲート） | Claude Code | docs/context.md を読み、フェーズの実装計画（目的・スコープ・実装手順・判断事項）を提示し、ユーザー承認後に Issue 作成・実装へ進む |
  ...
  | PR 作成 | Claude Code | **`doc-sync` スキルでドキュメント差異を確認 → `git status` でクリーンを確認 → PR 案をユーザーへ提示し承認を得てから作成する** |
