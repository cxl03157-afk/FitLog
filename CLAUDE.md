...
- フェーズの進め方：
  ...
  > **フェーズ開始時は必ず実装計画を先に提示すること。ユーザー承認なしに Issue 作成・実装へ進まない。**

  | ステップ | 担当 | 内容 |
  |---------|------|------|
  | 計画（必須ゲート） | Claude Code | docs/context.md・docs/phase-roadmap.md を確認し、**doc-sync を実行**して今回のフェーズに関係する実装・ドキュメントの差異を特定する（スコープ外の差異は別課題候補として報告し同フェーズに含めない）。更新が見込まれるドキュメントを明記した実装計画を提示する。複数ステップの更新処理、または DB・S3・外部 API・メール・キュー・ファイル処理など複数の副作用にまたがり部分成功や不整合が起こり得る場合は、計画内に「異常系設計」を含める。**異常系設計が未提示の場合は Issue 作成・実装へ進まない。** ユーザー承認後に Issue 作成・実装へ進む |
  ...
  | 仕様変更ゲート | Claude Code | HTTP メソッド・API パス・リクエスト/レスポンス・ファイル制約・DB スキーマ・認証認可・エラー仕様を承認済み計画または既存ドキュメントから変更する場合は作業を止め、変更理由・影響範囲・更新対象ドキュメントを提示してユーザー承認を得てから変更する |
  | テスト報告 | Claude Code | **`/quality-check` の完了報告前チェックをすべて満たしてから、規定フォーマットで結果をユーザーへ報告する** |
  | セルフレビュー | Claude Code | `/code-review` で diff を確認、指摘を解消 |
  | PR 作成 | Claude Code | **最終 doc-sync を実行 → `quality-check.md` 所定の doc-sync 報告フォーマットで結果を提示（未解消差異がある場合はユーザーの明示許可なく PR 案へ進まない）→ `git status` でクリーンを確認 → PR 案をユーザーへ提示し承認を得てから作成する** |
...
- `docs/phase-roadmap.md` はフェーズ完了時に該当フェーズの状態欄を「完了」に更新する
