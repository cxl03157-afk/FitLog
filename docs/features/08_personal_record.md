...
| weightKg | 条件付き | MAX_WEIGHT 時は必須。小数点以下2桁・0より大きい（最小 0.01kg）。0 は新規登録・更新とも拒否 |
...
| 項目 | 新規登録 | 編集 |
|------|---------|------|
| 種目 | セレクト（`{name}（{category}）` 形式）・**未選択状態で開く**（「種目を選択してください」）・明示選択が必須 | read-only テキスト表示（変更不可・既存種目を表示） |
...
- `weightKg`: 0.01 以上の有限数（空欄 / 0 / 負数 / 小数点以下3桁以上 / 不正文字列 → API 呼ばない）
- `reps`: 1 以上の整数（空欄 / 小数 / 0以下 / 不正文字列 → API 呼ばない）

> **既存データについて（Phase 13-2 時点）:** DB には 0kg の記録が存在する可能性がある。既存レコードは変更しない（読み取り・表示は可能）。DB の CHECK 制約は今回追加していない。既存データの整理または DB 制約追加は将来判断。
