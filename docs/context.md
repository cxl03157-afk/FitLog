...
- Status: Sub-phase 11-1（バックエンド修正）コミット完了。次は Sub-phase 11-2（Recharts 導入 + フロントエンド型定義・API クライアント）。
...
Sub-phase 11-1 コミット完了 → Sub-phase 11-2（`cd frontend && npm install recharts` + 型定義・API クライアント）開始。
